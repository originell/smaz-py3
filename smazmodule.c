...
py_smaz_compress(PyObject *self, PyObject *args)
{
    ...
    int compressed_size = smaz_compress(uncompressed_input, strlen(uncompressed_input), output_buffer, output_buffer_size);
    /*
    ...
    while (output_buffer_size + 1 == compressed_size)
    {
        ...
        compressed_size = smaz_compress(uncompressed_input, strlen(uncompressed_input), output_buffer, output_buffer_size);
    }
    /*
...
static PyObject *
py_smaz_decompress(PyObject *self, PyObject *args)
{
    ...
    char *output_buffer = (char *)malloc(output_buffer_size);
    int decompressed_size = smaz_decompress(compressed_data, compressed_data_size, output_buffer, output_buffer_size);
    /* If the compressed string didn't fit into the first 4096, we will get 4097
    ...
    while (output_buffer_size + 1 == decompressed_size)
    {
        ...
        // try again. the while() on top then checks if it did fit this time around.
        decompressed_size = smaz_decompress(compressed_data, compressed_data_size, output_buffer, output_buffer_size);
    }
    /*
...
static PyMethodDef SmazMethods[] = {
    {"compress", py_smaz_compress, METH_VARARGS,
     "Compresses a string using SMAZ compression.\n\nArgs:\n    string: An input string\nReturns:\n    bytes: The input string compressed via SMAZ compression."},
    {"decompress", py_smaz_decompress, METH_VARARGS,
     "Decompresses a SMAZ compressed string.\n\nArgs:\n    bytes: A SMAZ compressed input string\nReturns:\n    string: The input string decompressed via SMAZ decompression."},
